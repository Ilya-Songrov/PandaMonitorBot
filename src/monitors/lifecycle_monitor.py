...
class LifecycleMonitor:
    ...
    def _save_data(self):
        ...
        try:
            data = {"last_activity": datetime.now().isoformat(), "computer_name": settings.COMPUTER_NAME}
    ...
    def _add_session_start(self):
        ...
        session = {"startup": self.current_startup_time.isoformat(), "shutdown": None, "computer_name": settings.COMPUTER_NAME}
    ...
    def _calculate_today_statistics(self) -> Dict:
        ...
        for session in sessions:
            ...
            if startup.date() == today:
                ...
                # Track last activity
                end_time = datetime.fromisoformat(session["shutdown"]) if session.get("shutdown") else datetime.now()
    ...
    def _format_startup_message(self, stats: Dict) -> str:
        ...
        # Prepare previous shutdown line if available
        previous_shutdown_line = ""
        if self.last_shutdown_time:
            previous_shutdown_line = f"📅 Попереднє вимкнення: {self.last_shutdown_time.strftime('%Y-%m-%d %H:%M:%S')}\n"

        # Place previous shutdown above current start
        return f"""

� {settings.COMPUTER_NAME} запущено!

{previous_shutdown_line}📅 Поточний старт: {current_time}
